...
for year in ${years[@]}; do
    for cat in ${catAH[@]}; do
    	echo $cat$year
	nohup python plots/plot.py -c ${cat} -y ${year} -p > ${cat}${year}_fjetPt1_noB2Bleadingfjet.out &
    	#nohup python plots/plot.py -c ${cat} -y ${year} -p > ${cat}${year}_ttDM_MChi1_MPhi100_scalar.out &
    done
    for cat in ${catSL[@]}; do
	echo $cat$year
	nohup python plots/plot.py -c ${cat} -y ${year} -p > ${cat}${year}_HTcorrectionsAndNbjetSFs.out &
	#nohup python plots/plot.py -c ${cat} -y ${year} -p > ${cat}${year}_ttDM_MChi1_MPhi100_scalar.out &
    done
done
