...
while :
do
    RUNDIR="ModuleCommonSkim_09222021_UL2018"
    if [ ! -d "${RUNDIR}" ]; then 
	echo "using ${RUNDIR}"
	break
    fi
...
cat > "${SUBMIT}" << EOF
executable = ./runhaddnanoUL2018.sh
output = ${RUNDIR}/runhaddnanoUL2018.stdoutD
error = ${RUNDIR}/runhaddnanoUL2018.stderr
log = ${RUNDIR}/runhaddnanoUL2018.condor_log
